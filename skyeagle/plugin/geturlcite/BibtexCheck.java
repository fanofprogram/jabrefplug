...
public class BibtexCheck {
	...
	private String parseFieldContent(String key) throws IOException {
		...
		while (((c = peek()) != ',') && (c != '}') && (c != ')')) {
			...
			if (_eof) {
				break;
				// throw new RuntimeException("Error in line " + line + ": EOF
				// in mid-string");
			}
			if (c == '"') {
	...
	public String ris2Bibtex(StringBuffer sb) throws IOException {
		...
		String[] entries = sb.toString().replaceAll("\u2013", "-").replaceAll("\u2014", "--").replaceAll("\u2015", "--")
				.split("ER  -.*\\n");
		...
		for (int i = 0; i < entries.length; i++) {
			...
			for (int j = 0; j < fields.length; j++) {
				...
				if (entry.length() < 6)
					...
					// val;
						...
						if (author.equals("")) // don't add " and " for the
												// first author
							author = val;
						else
							author += " and " + val;
					} else if (lab.equals("A2")) {
						if (editor.equals("")) // don't add " and " for the
												// first editor
							editor = val;
						else
							editor += " and " + val;
						...
						if ((parts.length > 1) && (parts[1].length() > 0)) {
							try {
								...
								// The month part is unparseable, so we ignore
								// it.
							}
						}
					}
					...
					// Added ID import 2005.12.01, Morten Alver:
						...
						hm.put("refid", val);
					// Added doi import (sciencedirect.com) 2011.01.10,
					// Alexander Hug <alexander@alexanderhug.info>
			...
			// create one here
			BibtexEntry b = new BibtexEntry(BibtexFields.DEFAULT_BIBTEXENTRY_ID, Globals.getEntryType(type)); // id
																												// assumes
																												// an
																												// existing
																												// database
